...
function Index() {
  ...
  return (
    <main className="cosmic-bg relative min-h-screen overflow-hidden">
      ...
      <section id="ecosystem" className="section-glow overflow-hidden py-28"><div className="mx-auto max-w-7xl px-5 text-center"><Kicker>FUN ECOSYSTEM</Kicker><h2 className="font-display text-5xl font-bold md:text-7xl">ONE COSMOS.<br/><span className="pearl-text">ONE CONNECTED ECOSYSTEM.</span></h2><div className="relative mx-auto mt-20 min-h-[620px] max-w-4xl"><div className="absolute left-1/2 top-1/2 grid size-44 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border border-glow bg-primary/20 shadow-cosmos"><div><Globe2 className="mx-auto mb-2 size-10 text-glow"/><strong className="font-display">FUN COSMOS</strong></div></div><div className="absolute left-1/2 top-1/2 size-[560px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-border animate-[orbit_70s_linear_infinite]"/>{ecosystem.map((name,i)=><div key={name} className="glass-panel absolute grid min-h-16 w-36 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-2xl p-3 text-xs font-bold" style={{left:orbitPositions[i]?.[0] ?? "50%",top:orbitPositions[i]?.[1] ?? "50%"}}>{name}</div>)}</div><p className="mx-auto max-w-2xl text-muted-foreground">FUN COSMOS có thể trở thành lớp trải nghiệm chung của toàn FUN Ecosystem.</p></div></section>
